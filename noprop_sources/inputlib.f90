...
! Position Center i
   ...
   allocate(Centers(i+1)%x(Centers(i+1)%npos),Centers(i+1)%y(Centers(i+1)%npos),Centers(i+1)%z(Centers(i+1)%npos))
   do ipos = 0, Centers(i+1)%npos-1
     ...
     k = getAttribute(PositionNode,"z"); read (k,*) Centers(i+1)%z(ipos+1)
   enddo
